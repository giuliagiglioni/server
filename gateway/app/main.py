...
@app.api_route("/llm/{full_path:path}", methods=["GET","POST","PUT","PATCH","DELETE"])
async def llm_proxy(full_path: str, request: Request, db: Session = Depends(get_db)):
    api_key_row = get_api_key_or_401(request, db)
    ...
    external_path = f"/llm/{full_path}"
    upstream_path = f"/v1/{full_path}"
    ...
    try:
        resp = await forward_to_vllm(request, upstream_path)
